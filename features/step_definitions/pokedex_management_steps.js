...
Then('I should see completion statistics for each pokédex', async function() {
  ...
  const statsElements = await this.findElements(By.css('[data-testid="box-stats"], .progress, .stats'));
...
Then('I should see a {string} input field', async function(fieldName) {
  if (fieldName.toLowerCase().includes('name')) {
    ...
    const input = await this.waitForElement(By.css(`input[name="${fieldName.toLowerCase()}"], input[placeholder*="${fieldName}"]`));
    expect(input).to.exist;
  }
});

Then('I should see a {string} textarea', async function(fieldName) {
  const textarea = await this.waitForElement(By.css(`textarea[name="${fieldName.toLowerCase()}"], textarea[placeholder*="${fieldName}"]`));
  expect(textarea).to.exist;
});

Then('I should see regional pokédex selection options', async function() {
  const regionOptions = await this.findElements(By.css('[data-testid="national-pokedex-radio"], [data-testid="regional-pokedex-radio"], [data-testid="regional-pokedex-select"]'));
  expect(regionOptions.length).to.be.greaterThan(0);
});

Then('I should see challenge type checkboxes', async function() {
  const challengeOptions = await this.findElements(By.css('[data-testid="shiny-hunt-checkbox"], [data-testid="origin-required-checkbox"], [data-testid="include-forms-checkbox"]'));
...
When('I select the {string} pokédex option', async function(regionName) {
  if (regionName.toLowerCase().includes('national')) {
    const regionOption = await this.waitForElementVisible(By.css('[data-testid="national-pokedex-radio"]'));
    await regionOption.click();
  } else if (regionName.toLowerCase().includes('regional')) {
    const regionOption = await this.waitForElementVisible(By.css('[data-testid="regional-pokedex-radio"]'));
    await regionOption.click();
  } else {
    const regionOption = await this.waitForElementVisible(By.xpath(`//*[contains(text(), '${regionName}')]/ancestor-or-self::label//input | //*[contains(text(), '${regionName}')]/input`));
    await regionOption.click();
  }
});
...
When('I check the {string} option', async function(optionName) {
  ...
  const option = await this.waitForElementVisible(selector.startsWith('//') ? By.xpath(selector) : By.css(selector));
...
Then('I should see {string}, {string}, and {string} statistics', async function(stat1, stat2, stat3) {
