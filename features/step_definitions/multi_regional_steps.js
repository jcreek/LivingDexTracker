...
Then('I should see tabs for {string}, {string}, and {string}', async function(tab1, tab2, tab3) {
  const tabs = await this.findElements(By.css('.region-tab, .sub-region-tab, [data-testid="region-tab"]'));
...
Then('I should see tabs for {string}, {string}, {string}, and {string}', async function(tab1, tab2, tab3, tab4) {
  const tabs = await this.findElements(By.css('.region-tab, .sub-region-tab, [data-testid="region-tab"]'));
...
When('I click the {string} tab', async function(tabName) {
  const tab = await this.waitForElementVisible(By.xpath(`//*[contains(text(), '${tabName}')][contains(@class, 'tab') or ancestor::*[contains(@class, 'tab')]]`));
  await tab.click();
});
...
Then('the Box View should recalculate for the coastal region', async function() {
  ...
  const boxIndicator = await this.findElements(By.css('.box-number, [data-testid="box-indicator"]'));
  expect(boxIndicator.length).to.be.greaterThan(0);
});

Then('the {string} tab should be highlighted as active', async function(tabName) {
  const activeTab = await this.waitForElement(By.xpath(`//*[contains(text(), '${tabName}')][contains(@class, 'active') or contains(@class, 'selected') or @aria-selected='true']`));
...
Then('I should see {int} boxes total \\({int} ÷ {int} = {int}\\)', async function(expectedBoxes, totalPokemon, pokemonPerBox, division) {
  ...
  console.log(`Expected ${expectedBoxes} boxes (${totalPokemon} ÷ ${pokemonPerBox} = ${division})`);
});
...
Then('I should see {int} boxes total \\({int} ÷ {int} = {float} → {int}\\)', async function(expectedBoxes, totalPokemon, pokemonPerBox, exactDivision, roundedUp) {
  await new Promise(resolve => setTimeout(resolve, 1000));
  
  const boxes = await this.findElements(By.css('.box, [data-testid="box"], .box-indicator'));
  expect(boxes.length).to.be.greaterThan(0);
  
  console.log(`Expected ${expectedBoxes} boxes (${totalPokemon} ÷ ${pokemonPerBox} = ${exactDivision} → ${roundedUp})`);
});
...
Then('I should only see results from the Coastal Kalos pokédex', async function() {
  ...
  const results = await this.findElements(By.css('.pokemon-item, .search-result, [data-testid="pokemon-result"]'));
