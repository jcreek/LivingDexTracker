...
When('I click the {string} navigation link', async function(linkText) {
  ...
  switch (linkText) {
    case 'Home':
      selector = '[data-testid="nav-home-desktop"], [data-testid="nav-home-mobile"]';
      break;
    case 'Pokédexes':
    case 'My Pokédexes':
  ...
  const navLink = await this.waitForElementVisible(selector.startsWith('//') ? By.xpath(selector) : By.css(selector));
  await navLink.click();
});
...
Then('the {string} navigation link should be highlighted as current', async function(linkText) {
  const currentLink = await this.waitForElement(By.xpath(`//nav//*[contains(text(), '${linkText}')]/ancestor-or-self::*[contains(@class, 'active') or contains(@class, 'current') or @aria-current]`));
