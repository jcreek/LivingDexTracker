...
Then('I should see user statistics', async function () {
	const statsElements = await this.findElements(
		By.css('[data-testid="stats-grid"], [data-testid="collection-stats-card"], .stats')
	);
...
Then('I should see overall completion statistics', async function () {
	const statsElements = await this.findElements(
		By.css('.completion, .percentage, .stats, [data-testid="completion-stats"]')
	);
	expect(statsElements.length).to.be.greaterThan(0);
});

Then('I should see individual pokédex progress', async function () {
	const progressElements = await this.findElements(
		By.css('.progress, .pokedex-progress, [data-testid="pokedex-progress"]')
	);
...
When('I click on a specific pokédex in the statistics', async function () {
	try {
		const pokedexLinks = await this.findElements(
			By.css('a[href*="mydex"], .pokedex-link, [data-testid="pokedex-link"]')
		);
...
Then("I should see that pokédex's detailed view", async function () {
	const detailView = await this.waitForElement(
		By.css('.box-view, .list-view, .pokemon-grid, .pokemon-list, main')
	);
