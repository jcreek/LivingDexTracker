...
Then('I should see a green background indicator', async function() {
  const greenIndicator = await this.findElements(By.css('.bg-green, .caught, [data-status="caught"]'));
  expect(greenIndicator.length).to.be.greaterThan(0);
});

Then('I should see a yellow background indicator', async function() {
  const yellowIndicator = await this.findElements(By.css('.bg-yellow, .ready-to-evolve, [data-status="ready_to_evolve"]'));
...
When('I click on the same ready-to-evolve Pokémon slot', async function() {
  const readySlot = await this.waitForElementVisible(By.css('.ready-to-evolve, [data-status="ready_to_evolve"], .pokemon-slot:first-child'));
...
Given('I have a caught Pokémon in the current box', async function() {
  ...
  try {
    const uncaughtSlot = await this.findElement(By.css('.uncaught, .empty-slot, .pokemon-slot:first-child'));
...
When('I right-click on the caught Pokémon', async function() {
  const caughtPokemon = await this.waitForElementVisible(By.css('.caught, [data-status="caught"], .pokemon-slot:first-child'));
  await this.driver.actions().contextClick(caughtPokemon).perform();
});

Then('I should see a catch management modal', async function() {
  const modal = await this.waitForElement(By.css('.modal, .catch-modal, [data-testid="catch-modal"]'));
  expect(modal).to.exist;
});

Then('I should see catch status options', async function() {
  const statusOptions = await this.findElements(By.css('.status-option, input[name="catch_status"], [data-testid="status-option"]'));
  expect(statusOptions.length).to.be.greaterThan(0);
});

Then('I should see catch location options', async function() {
  const locationOptions = await this.findElements(By.css('.location-option, input[name="catch_location"], [data-testid="location-option"]'));
  expect(locationOptions.length).to.be.greaterThan(0);
});

Then('I should see a notes textarea', async function() {
  const notesTextarea = await this.waitForElement(By.css('textarea[name="notes"], textarea[placeholder*="notes"], [data-testid="notes-textarea"]'));
...
Then('I should see Gigantamax checkbox if applicable', async function() {
  ...
  const gigantamaxCheckbox = await this.findElements(By.css('input[name="is_gigantamax"], [data-testid="gigantamax-checkbox"]'));
...
Given('I have the catch management modal open', async function() {
  // Try to open the modal if not already open
  try {
    const caughtPokemon = await this.findElement(By.css('.caught, [data-status="caught"], .pokemon-slot:first-child'));
    await this.driver.actions().contextClick(caughtPokemon).perform();
    await new Promise(resolve => setTimeout(resolve, 500));
  } catch (error) {
    console.log('Modal might already be open or no caught Pokémon available');
  }
});

When('I select {string} as the catch location', async function(location) {
  const locationOption = await this.waitForElementVisible(By.xpath(`//*[contains(text(), '${location}')]/ancestor-or-self::label//input | //input[@value='${location.toLowerCase().replace(' ', '_')}']`));
...
When('I enter {string} in the notes field', async function(noteText) {
  const notesField = await this.waitForElementVisible(By.css('textarea[name="notes"], textarea[placeholder*="notes"], [data-testid="notes-textarea"]'));
...
When('I reopen the modal for the same Pokémon', async function() {
  ...
  try {
    const closeButton = await this.findElement(By.css('.modal-close, .close, [data-testid="close-modal"]'));
  ...
  // Reopen the modal
  const pokemon = await this.waitForElementVisible(By.css('.caught, [data-status="caught"], .pokemon-slot:first-child'));
  await this.driver.actions().contextClick(pokemon).perform();
});

Then('I should see my saved notes', async function() {
  const notesField = await this.waitForElement(By.css('textarea[name="notes"], textarea[placeholder*="notes"], [data-testid="notes-textarea"]'));
...
When('I check the {string} checkbox', async function(checkboxName) {
  const checkbox = await this.waitForElementVisible(By.xpath(`//*[contains(text(), '${checkboxName}')]/ancestor-or-self::label//input | //input[@name='${checkboxName.toLowerCase()}']`));
...
Then('the Pokémon should show a Gigantamax indicator', async function() {
  const gigantamaxIndicator = await this.findElements(By.css('.gigantamax-icon, [data-testid="gigantamax-indicator"]'));
...
When('I select {string} as the origin region', async function(region) {
  const regionSelect = await this.waitForElementVisible(By.css('select[name="origin_region"], [data-testid="origin-region-select"]'));
  await regionSelect.click();
  const regionOption = await this.waitForElementVisible(By.xpath(`//option[contains(text(), '${region}')]`));
  await regionOption.click();
});

When('I select {string} as the game caught in', async function(game) {
  const gameSelect = await this.waitForElementVisible(By.css('select[name="game_caught_in"], [data-testid="game-select"]'));
  await gameSelect.click();
  const gameOption = await this.waitForElementVisible(By.xpath(`//option[contains(text(), '${game}')]`));
...
Then('I should see origin indicators if required by pokédex settings', async function() {
  const originIndicators = await this.findElements(By.css('.origin-indicator, [data-testid="origin-indicator"]'));
...
Given('I have selected {int} Pokémon in the current box', async function(count) {
  const pokemonSlots = await this.findElements(By.css('.pokemon-slot, .box-slot, [data-testid="pokemon-slot"]'));
  
  for (let i = 0; i < Math.min(count, pokemonSlots.length); i++) {
    await this.driver.actions().keyDown(Key.CONTROL).click(pokemonSlots[i]).keyUp(Key.CONTROL).perform();
  }
});

When('I choose {string} from bulk actions', async function(actionName) {
  const bulkAction = await this.waitForElementVisible(By.xpath(`//button[contains(text(), '${actionName}')]`));
  await bulkAction.click();
});

Then('all {int} selected Pokémon should show yellow backgrounds', async function(count) {
  await new Promise(resolve => setTimeout(resolve, 1000));
  const yellowPokemon = await this.findElements(By.css('.ready-to-evolve, [data-status="ready_to_evolve"]'));
...
When('I select all Pokémon in the current box', async function() {
  const selectAllButton = await this.findElements(By.css('.select-all, [data-testid="select-all"]'));
...
Then('the previously ready-to-evolve Pokémon should remain as {string}', async function(status) {
  const readyToEvolve = await this.findElements(By.css('.ready-to-evolve, [data-status="ready_to_evolve"]'));
...
// Statistics accuracy
Given('I have {int} caught, {int} ready-to-evolve, and {int} uncaught Pokémon', async function(caught, readyToEvolve, uncaught) {
  console.log(`Test setup: ${caught} caught, ${readyToEvolve} ready-to-evolve, ${uncaught} uncaught`);
});
...
Then('the ready-to-evolve count should show {string}', async function(expectedCount) {
  const readyCount = await this.findElements(By.css('.ready-to-evolve, [data-status="ready_to_evolve"]'));
...
Then('the completion percentage should calculate correctly', async function() {
  const percentageElement = await this.findElements(By.css('.percentage, .completion-rate, [data-testid="completion-percentage"]'));
...
Then('I should see yellow backgrounds for ready-to-evolve Pokémon', async function() {
  const yellowBackgrounds = await this.findElements(By.css('.ready-to-evolve, [data-status="ready_to_evolve"]'));
...
Then('I should see Gigantamax symbols where applicable', async function() {
  const gigantamaxSymbols = await this.findElements(By.css('.gigantamax-icon, [data-testid="gigantamax-indicator"]'));
