	...
	{#if pokedexes.length > 0}
		<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
			{#each pokedexes as pokedex}
				<PokedexCard {pokedex} on:delete={handleDeletePokedex} canDelete={pokedexes.length > 1} />
			{/each}
		</div>
	{:else}
		<div class="text-center py-16">
			...
			<p class="text-xl text-gray-600 mb-6">
				Create your first pokédex to start tracking your collection!
			</p>
...
{#if showCreateModal}
	<CreatePokedexModal on:create={handleCreatePokedex} on:cancel={() => (showCreateModal = false)} />
{/if}
